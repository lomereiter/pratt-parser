...
void PrettyPrinter::PrettyPrinter::visit(const std::shared_ptr<Node>& e) {
    indent() << "IMPLEMENT ME!" << '\n';
}
...
void PrettyPrinter::visit(const std::shared_ptr<IndexTypeNode>& e) { travel(e -> type); }
...
void PrettyPrinter::indented(const std::function<void()>& func) {
    _indent += sw; func(); _indent -= sw;
}

void PrettyPrinter::noindent(const std::function<void()>& func) {
